...
import 'package:flutter_weather_app/generic_widgets/radial_position.dart';
...
class SlidingRadialListController extends ChangeNotifier {
  ...
  Completer onOpendedCompleter;
  Completer onClosedCompleter;

  SlidingRadialListController({
      this.itemCount,
      vsync,
    }) : _slideController = new AnimationController(vsync: vsync, duration: const Duration(milliseconds: 1500)),
  _fadeController = new AnimationController(vsync: vsync, duration: const Duration(milliseconds: 1500)),
  _slidePositions = [] {
    _slideController
  ...
  Future open() {
    if(_state == RadialListState.closed) {
      ...
      onOpendedCompleter = new Completer();
      return onOpendedCompleter.future;
    }
...
class RadialListItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final circleDecoration = listItem.isSelected ? new BoxDecoration(
        ...
        children: <Widget>[
          new Container(
            ...
            child: new Padding(
              ...
              child: new Image(
                image: listItem.icon,
              color: listItem.isSelected ? const Color(0xFF66BBCC) : Colors.white,
              ),
            ),
          ),
          new Padding(
